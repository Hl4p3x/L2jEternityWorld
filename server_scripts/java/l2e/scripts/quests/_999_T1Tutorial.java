...
public class _999_T1Tutorial extends Quest
{
	...
	public String onAdvEvent(String event, L2Npc npc, L2PcInstance player)
	{
		...
		else if (event.equalsIgnoreCase("isle"))
		{
			...
			htmltext = "<html><body>" + npc.getName() + ":<br>" + LocalizationStorage.getInstance().getString(player.getLang(), "999quest.ISLE") + "</body></html>";
		}
		else
		{
					...
					else if ((step == 1) && (st.getQuestItemsCount(talk.item) == 0) && (Ex <= 2))
					{
						if (st.getQuestItemsCount(BLUE_GEM) != 0)
						{
							...
							if (player.getClassId().isMage())
							{
								...
								if (htmltext.equals(""))
								{
									htmltext = "<html><body>" + LocalizationStorage.getInstance().getString(player.getLang(), "999quest.CAN_NOT_HELP") + "</body></html>";
								}
							}
							else
							{
								...
								if (htmltext.equals(""))
								{
									htmltext = "<html><body>" + LocalizationStorage.getInstance().getString(player.getLang(), "999quest.CAN_NOT_HELP1") + "</body></html>";
								}
							}
						}
						else
