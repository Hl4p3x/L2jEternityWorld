...
public class Auction
{
	...
	private void returnItem(String Clan, long quantity, boolean penalty)
	{
		...
		final long limit = MAX_ADENA - ClanHolder.getInstance().getClanByName(Clan).getWarehouse().getAdena();
		quantity = Math.min(quantity, limit);
		
		ClanHolder.getInstance().getClanByName(Clan).getWarehouse().addItem("Outbidded", ADENA_ID, quantity, null, null);
	}
	...
	private void removeBids()
	{
		...
		for (Bidder b : _bidders.values())
		{
			...
			{
				returnItem(b.getClanName(), b.getBid(), true);
			}
			else
			{
	...
	public void endAuction()
	{
		if (ClanHallManager.getInstance().loaded())
		{
			...
			deleteAuctionFromDB();
			L2Clan Clan = ClanHolder.getInstance().getClanByName(_bidders.get(_highestBidderId).getClanName());
	...
	public synchronized void cancelBid(int bidder)
	{
		...
		ClanHolder.getInstance().getClanByName(_bidders.get(bidder).getClanName()).setAuctionBiddedAt(0, true);
		_bidders.clear();
		loadBid();
	}
