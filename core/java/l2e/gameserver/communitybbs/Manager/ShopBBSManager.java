...
/**
 * Created by LordWinter 08.07.2013 Fixed by L2J Eternity-World
 */
public class ShopBBSManager extends BaseBBSManager
{
	private final static Logger _log = Logger.getLogger(ShopBBSManager.class.getName());
	...
	public void parsecmd(String command, L2PcInstance activeChar)
	{
		...
		{
			sendHtm(activeChar, "data/html/CommunityBoard/6.htm");
			activeChar.setSessionVar("add_fav", command + "&" + "" + LocalizationStorage.getInstance().getString(activeChar.getLang(), "FavoriteBBS.SHOPMAIN") + "");
		}
		else if (command.equalsIgnoreCase("_bbsshop;adena"))
		{
			sendHtm(activeChar, "data/html/CommunityBoard/7.htm");
			activeChar.setSessionVar("add_fav", command + "&" + "" + LocalizationStorage.getInstance().getString(activeChar.getLang(), "FavoriteBBS.SHOPADENA") + "");
		}
		else
		{
