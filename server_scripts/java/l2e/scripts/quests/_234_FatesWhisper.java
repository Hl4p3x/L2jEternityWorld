...
{
	private static final String qn = "_234_FatesWhisper";
	...
	{
	                31002, 30182, 30847, 30178, 30833, 31028, 31029, 31030, 31027
	};
		...
		else if (event.startsWith("selectAGrade_"))
		{
			if (st.getInt("bypass") == 1)
			{
				...
				if (st.hasQuestItems(itemId))
				{
					...
					htmltext = st.showHtmlFile("31002-12.htm").replace("%weaponname%", ItemHolder.getInstance().getTemplate(aGradeItemId).getName());
