...
import static l2e.gameserver.util.Util.formatAdena;
...
public class PremiumBuffer extends Quest
{
	...
	private static final int NPC_ID = Config.NpcBuffer_ID;
	...
	{
		return "<html><head><title>" + TITLE_NAME + "</title></head><body><center><img src=\"L2UI_CH3.herotower_deco\" width=256 height=32><br>" + "<font color=303030>" + TITLE_NAME + "</font><br>" + "<img src=\"L2UI.SquareGray\" width=250 height=1><br>" + "<table width=260 border=0 bgcolor=444444>" + "<tr><td><br></td></tr>"
		                + "<tr><td align=center><font color=FFFFFF>This option can be seen by GMs only and it<br1>allow to update any changes made in the<br1>script. You can disable this option in<br1>the settings section within the Script.<br><font color=LEVEL>Do you want to update the SCRIPT?</font></font></td></tr>" + "<tr><td></td></tr></table><br>"
		                + "<img src=\"L2UI.SquareGray\" width=250 height=1><br><br>" + "<button value=\"Yes\" action=\"bypass -h Quest " + QUEST_LOADING_INFO + " reloadscript 1 0 0\" width=50 height=25 back=\"L2UI_ct1.button_df\" fore=\"L2UI_ct1.button_df\">" + "<button value=\"No\" action=\"bypass -h Quest " + QUEST_LOADING_INFO
		                + " reloadscript 0 0 0\" width=50 height=25 back=\"L2UI_ct1.button_df\" fore=\"L2UI_ct1.button_df\">" + "</center></body></html>";
	}
	...
	{
		return "<html><head><title>" + TITLE_NAME + "</title></head><body><center><img src=\"L2UI_CH3.herotower_deco\" width=256 height=32><br><br>You MUST seprerate new words with a dot (.)<br><br>Scheme name: <edit var=\"name\" width=100><br><br>" + "<button value=\"Create Scheme\" action=\"bypass -h Quest " + QUEST_LOADING_INFO
		                + " create $name no_name x x\" width=200 height=20 back=\"L2UI_ct1.button_df\" fore=\"L2UI_ct1.button_df\">" + "<br><button value=\"Back\" action=\"bypass -h Quest " + QUEST_LOADING_INFO + " redirect main 0 0\" width=100 height=20 back=\"L2UI_ct1.button_df\" fore=\"L2UI_ct1.button_df\">" + "<br><font color=303030>" + TITLE_NAME + "</font></center></body></html>";
	}
	...
	private String getOptionList(String scheme)
	{
		...
		HTML += "<br><button value=\"Back\" action=\"bypass -h Quest " + QUEST_LOADING_INFO + " edit_1 0 0 0\" width=100 height=20 back=\"L2UI_ct1.button_df\" fore=\"L2UI_ct1.button_df\">" + "<button value=\"Home\" action=\"bypass -h Quest " + QUEST_LOADING_INFO + " redirect main 0 0\" width=100 height=20 back=\"L2UI_ct1.button_df\" fore=\"L2UI_ct1.button_df\">"
	...
	private String buildHtml(String buffType)
	{
		...
		try (Connection con = L2DatabaseFactory.getInstance().getConnection())
		{
			...
			while (rs.next())
			{
				...
				String bName = SkillHolder.getInstance().getInfo(bId, bLevel).getName();
				bName = bName.replace(" ", "+");
				availableBuffs.add(bName + "_" + bId + "_" + bLevel);
				// except: HTML_MESSAGE +=
				// "Error loading buff list...<br>"
			}
		}
		catch (SQLException e)
		{
	...
	private String viewAllSchemeBuffs(String scheme, String page, String action)
	{
		...
		try (Connection con = L2DatabaseFactory.getInstance().getConnection())
		{
			if (action.equals("add"))
			{
				...
				while (rss.next())
				{
					String name = SkillHolder.getInstance().getInfo(rss.getInt("buffId"), rss.getInt("buffLevel")).getName();
					name = name.replace(" ", "+");
			...
			else if (action.equals("remove"))
			{
				...
				while (rss.next())
				{
					String name = SkillHolder.getInstance().getInfo(rss.getInt("skill_id"), rss.getInt("skill_level")).getName();
					name = name.replace(" ", "+");
		...
		HTML_MESSAGE += "<br><br><button value=\"Back\" action=\"bypass -h Quest " + QUEST_LOADING_INFO + " manage_scheme_select " + scheme + " x x\" width=100 height=20 back=\"L2UI_ct1.button_df\" fore=\"L2UI_ct1.button_df\">" + "<button value=\"Home\" action=\"bypass -h Quest " + QUEST_LOADING_INFO
	...
	private String viewAllBuffs(String type, String typeName, String page)
	{
		...
		for (int i = start; i < end; ++i)
		{
			...
			if (type.equals("set"))
			{
				...
				HTML_MESSAGE += "<tr><td fixwidth=145>" + name + "</td><td width=70><combobox var=\"newSet" + i + "\" width=70 " + listOrder + "></td>" + "<td width=50><button value=\"Update\" action=\"bypass -h Quest " + QUEST_LOADING_INFO + " changeBuffSet " + skillPos + " $newSet" + i + " " + page
		...
		HTML_MESSAGE += "<br><br><button value=\"Back\" action=\"bypass -h Quest " + QUEST_LOADING_INFO + " redirect manage_buffs 0 0\" width=100 height=20 back=\"L2UI_ct1.button_df\" fore=\"L2UI_ct1.button_df\">" + "<button value=\"Home\" action=\"bypass -h Quest " + QUEST_LOADING_INFO + " redirect main 0 0\" width=100 height=20 back=\"L2UI_ct1.button_df\" fore=\"L2UI_ct1.button_df\">"
		                + "<br><font color=303030>" + TITLE_NAME + "</font></center></body></html>";
		return HTML_MESSAGE;
	}
	...
	{
		// TRACE("onAdvEvent('"+event+"',"+(npc == null ? "NULL" :
		// npc.getId()+npc.getName())+","+(player == null ? "NULL" :
		// player.getName())+")");/*@IF DEBUG@*/
		if (DEBUG)
		{
		...
		switch (eventParam0)
		{
			...
			case "delete_c":
				return "<html><head><title>" + TITLE_NAME + "</title></head><body><center><img src=\"L2UI_CH3.herotower_deco\" width=256 height=32><br>Do you really want to delete '" + eventParam2 + "' scheme?<br><br>" + "<button value=\"Yes\" action=\"bypass -h Quest " + QUEST_LOADING_INFO + " delete " + eventParam1
				                + " x x\" width=50 height=25 back=\"L2UI_ct1.button_df\" fore=\"L2UI_ct1.button_df\">" + "<button value=\"No\" action=\"bypass -h Quest " + QUEST_LOADING_INFO + " delete_1 x x x\" width=50 height=25 back=\"L2UI_ct1.button_df\" fore=\"L2UI_ct1.button_df\">" + "<br><font color=303030>" + TITLE_NAME + "</font></center></body></html>";
			...
			case "cast":
				if ((int) (System.currentTimeMillis() / 1000) > st.getInt("blockUntilTime"))
				{
					...
					for (int i = 0; i < buffs.size(); ++i)
					{
						if (!getSummonbuff)
						{
							SkillHolder.getInstance().getInfo(buffs.get(i), levels.get(i)).getEffects(player, player);
						}
						else
						{
							if (player.getSummon() != null)
							{
								SkillHolder.getInstance().getInfo(buffs.get(i), levels.get(i)).getEffects(player.getSummon(), player.getSummon());
							}
							else
							{
								return showText(st, "Info", "You can't use the Pet's options.<br>Summon your pet first!", false, "Return", "main");
			...
			case "giveBuffs":
			{
				...
				if ((int) (System.currentTimeMillis() / 1000) > st.getInt("blockUntilTime"))
				{
					...
					L2Skill skill = SkillHolder.getInstance().getInfo(Integer.parseInt(eventParam1), Integer.parseInt(eventParam2));
					...
					if (!getSummonbuff)
					{
						if (eventParam3.equals("cubic"))
						{
							...
							player.useMagic(SkillHolder.getInstance().getInfo(Integer.parseInt(eventParam1), Integer.parseInt(eventParam2)), false, false);
						}
						else
						{
							SkillHolder.getInstance().getInfo(Integer.parseInt(eventParam1), Integer.parseInt(eventParam2)).getEffects(player, player);
						}
					}
					else
					{
						...
						else
						{
							if (player.getSummon() != null)
							{
								SkillHolder.getInstance().getInfo(Integer.parseInt(eventParam1), Integer.parseInt(eventParam2)).getEffects(player.getSummon(), player.getSummon());
							}
							else
							{
								return showText(st, "Info", "You can't use the Pet's options.<br>Summon your pet first!", false, "Return", "main");
			...
			case "castBuffSet":
				if ((int) (System.currentTimeMillis() / 1000) > st.getInt("blockUntilTime"))
				{
					...
					if (!getSummonbuff)
					{
						...
						for (int[] i : buff_sets)
						{
							SkillHolder.getInstance().getInfo(i[0], i[1]).getEffects(player, player);
						}
					}
					else
					{
						if (player.getSummon() != null)
						{
							...
							for (int[] i : buff_sets)
							{
								SkillHolder.getInstance().getInfo(i[0], i[1]).getEffects(player.getSummon(), player.getSummon());
							}
						}
						else
						{
	...
	public String onFirstTalk(L2Npc npc, L2PcInstance player)
	{
		...
		{
			// BUFF ONLY FOR VIP - ENABLE_VIP_BUFFER = must be
			// enabled
			if (Config.VIP_ONLY)
			{
				if ((ENABLE_VIP_BUFFER) && (player.getPremiumService() == 1))
				{
			...
			// BUFF FOR ALL
			else if ((ENABLE_VIP_BUFFER) && (player.getPremiumService() == 1))
			{
				if (!BUFF_WITH_KARMA && (player.getKarma() > 0))
				{
					return showText(st, "Info", "You have too much <font color=FF0000>karma!</font><br>Come back,<br>when you don't have any karma!", false, "Return", "main");
			...
			else if ((ENABLE_VIP_BUFFER) && (player.getPremiumService() == 0))
			{
				return rebuildMainHtml(st);
			}
