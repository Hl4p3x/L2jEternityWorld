...
public class L2PhantomMageAI extends L2PlayableAI
{
	private boolean _thinking;
	private boolean _cpTask = false;
	private boolean _healTask = false;
	
	public L2PhantomMageAI(AIAccessor accessor)
	{
		super(accessor);
	}
	...
	protected void onEvtFinishCasting()
	{
		...
		else
		{
			...
			{
				setIntention(CtrlIntention.AI_INTENTION_CAST, _skill, _actor.getTarget());
			}
		}
	}
	...
	protected void onEvtAttacked(L2Character attacker)
	{
		...
		if (_actor.getCurrentMp() >= _actor.getStat().getMpConsume(_skill))
		{
			setIntention(CtrlIntention.AI_INTENTION_CAST, _skill, attacker);
		}
		else
		{
	...
	private void thinkAttack()
	{
		...
		if (_actor.getCurrentMp() >= _actor.getStat().getMpConsume(_skill))
		{
			setIntention(CtrlIntention.AI_INTENTION_CAST, _skill, target);
		}
		else
		{
