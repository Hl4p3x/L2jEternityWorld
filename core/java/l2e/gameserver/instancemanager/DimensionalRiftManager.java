...
public final class DimensionalRiftManager
{
	...
	public void loadSpawns()
	{
		...
		try
		{
			...
			File file = new File(Config.DATAPACK_ROOT, "data/spawnZones/dimensionalRift.xml");
			if (!file.exists())
			{
	...
	public synchronized void start(L2PcInstance player, byte type, L2Npc npc)
	{
		...
		if (player.getParty().getMemberCount() < Config.RIFT_MIN_PARTY_SIZE)
		{
			...
			html.setFile(player.getLang(), "data/html/seven_signs/rift/SmallParty.htm");
		...
		{
			NpcHtmlMessage html = new NpcHtmlMessage(npc.getObjectId());
			html.setFile(player.getLang(), "data/html/seven_signs/rift/NoFragments.htm");
			html.replace("%npc_name%", npc.getName());
