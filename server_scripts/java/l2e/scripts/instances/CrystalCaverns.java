...
public class CrystalCaverns extends Quest
{
	...
	private static final int INSTANCEID = 10;
	...
	private void Throw(L2Character effector, L2Character effected)
	{
		...
		if (Config.GEODATA)
		{
			Location destiny = GeoClient.getInstance().moveCheck(effected.getX(), effected.getY(), effected.getZ(), _x, _y, _z, true);
			_x = destiny.getX();
			_y = destiny.getY();
		}
	...
	protected int enterInstance(L2PcInstance player, String template, Location loc)
	{
		...
		else
		{
			for (L2PcInstance partyMember : party.getMembers())
			{
				...
				teleportPlayer(partyMember, loc, world.instanceId);
	...
	public String onAdvEvent(String event, L2Npc npc, L2PcInstance player)
	{
		...
		if (tmpworld instanceof CCWorld)
		{
			...
			{
				teleportPlayer(player, new Location(153689, 142226, -9750), world.instanceId);
			}
			else if (event.equalsIgnoreCase("Timer2") || event.equalsIgnoreCase("Timer3") || event.equalsIgnoreCase("Timer4") || event.equalsIgnoreCase("Timer5"))
			{
				...
				{
					teleportPlayer(player, new Location(144653, 152606, -12126), world.instanceId);
					player.stopSkillEffects(5239);
					SkillHolder.getInstance().getInfo(5239, 1).getEffects(player, player);
			...
			else if (event.equalsIgnoreCase("spawn_oracle"))
			{
				...
				addSpawn((getRandom(10) < 5 ? 29116 : 29117), npc.getX(), npc.getY(), npc.getZ(), npc.getHeading(), false, 0, false, world.instanceId); // Baylor's
																					// Chest
			...
			else if (event.equalsIgnoreCase("baylor_alarm"))
			{
				if (world._alarm == null)
				{
					...
					npc.addSkill(SkillHolder.getInstance().getInfo(5244, 1));
					npc.addSkill(SkillHolder.getInstance().getInfo(5245, 1));
			...
			else if (event.equalsIgnoreCase("baylor_skill"))
			{
				...
				else
				{
					...
					if ((nowHp < (maxHp * 0.2)) && (world._raidStatus < 3) && (npc.getFirstEffect(5224) == null) && (npc.getFirstEffect(5225) == null))
					{
						...
						{
							npc.doCast(SkillHolder.getInstance().getInfo(5225, 1));
					...
					{
						npc.doCast(SkillHolder.getInstance().getInfo(5224, 1));
					}
					else if (rand < 33)
					{
						npc.setTarget(world._raiders.get(getRandom(world._raiders.size())));
						npc.doCast(SkillHolder.getInstance().getInfo(5229, 1));
					}
				}
			}
				...
				else if (npc.getId() == GK2)
				{
					...
					if (party != null)
					{
						for (L2PcInstance partyMember : party.getMembers())
						{
							...
							{
								SkillHolder.getInstance().getInfo(5239, 1).getEffects(partyMember, partyMember);
					...
					case 32275:
						...
						if (party != null)
						{
							for (L2PcInstance partyMember : party.getMembers())
							{
								...
								{
									partyMember.stopSkillEffects(5239);
									SkillHolder.getInstance().getInfo(5239, 2).getEffects(partyMember, partyMember);
						...
						{
							player.stopSkillEffects(5239);
							SkillHolder.getInstance().getInfo(5239, 2).getEffects(player, player);
					...
					case 32276:
						...
						if (party != null)
						{
							for (L2PcInstance partyMember : party.getMembers())
							{
								...
								{
									partyMember.stopSkillEffects(5239);
									SkillHolder.getInstance().getInfo(5239, 4).getEffects(partyMember, partyMember);
						...
						{
							player.stopSkillEffects(5239);
							SkillHolder.getInstance().getInfo(5239, 4).getEffects(player, player);
					...
					case 32277:
						...
						if (party != null)
						{
							for (L2PcInstance partyMember : party.getMembers())
							{
								...
								{
									partyMember.stopSkillEffects(5239);
									SkillHolder.getInstance().getInfo(5239, 3).getEffects(partyMember, partyMember);
						...
						{
							player.stopSkillEffects(5239);
							SkillHolder.getInstance().getInfo(5239, 3).getEffects(player, player);
