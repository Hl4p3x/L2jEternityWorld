...
public class AdminCreateItem implements IAdminCommandHandler
{
	...
	public boolean useAdminCommand(String command, L2PcInstance activeChar)
	{
		...
		{
			adminhtm.setFile(activeChar.getLang(), "data/html/admin/itemcreation.htm");
		...
		else if (command.startsWith("admin_give_item_to_all"))
		{
			...
			int counter = 0;
			L2Item template = ItemHolder.getInstance().getTemplate(idval);
			if (template == null)
			{
	...
	{
		L2Item template = ItemHolder.getInstance().getTemplate(id);
		if (template == null)
		{
			activeChar.sendMessage("This item doesn't exist.");
			return;
